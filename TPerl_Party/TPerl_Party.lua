...
-- Party power can be returned as a secret value; avoid compares/arithmetics on it.
local canaccessvalue = canaccessvalue
local issecretvalue = issecretvalue

local function TPerl_Party_CanAccess(v)
...
local function TPerl_Party_SafeBool(v)
	-- Convert possibly-secret booleans into a safe Lua boolean.
	if (not IsRetail) then
		return (v and true or false)
	end
	if (not TPerl_Party_CanAccess(v)) then
		return false
	end
...
local function TPerl_Party_TrySetPowerBar(bar, curVal, maxVal)
	if (not bar) then
		return false
	end
	if (not IsRetail) then
		bar:SetMinMaxValues(0, maxVal or 0)
		bar:SetValue(curVal or 0)
		return true
	end
	-- Midnight/Retail: UnitPower values can be "secret". Some UI methods accept them, some don't.
	-- Try to paint the bar without doing comparisons/arithmetics on the values.
	local ok = pcall(bar.SetMinMaxValues, bar, 0, maxVal)
	if ok then
		ok = pcall(bar.SetValue, bar, curVal)
	end
	if not ok then
		-- Fallback to a safe empty bar
...
local function onAttrChanged(self, name, value)
	if (name == "unit") then
		-- Retail/Midnight: SecureGroupHeader can briefly report "party0" during reconfig.
		-- If showPlayer is disabled, treating "party0" as a real unit will cause frames to
		-- lose their unit binding and stop receiving aura updates. Ignore this transient value.
		if IsRetail and value == "party0" then
			return
		end
		if (value) then
...
local function TPerl_Party_UpdateHealth(self)
	...
	if (not UnitIsConnected(partyid)) then
		...
		elseif (UnitIsGhost(partyid)) then
			...
			elseif (not IsRetail and (Partyhealth == 1) and (Partyhealthmax == 1)) then
				reason = TPERL_LOC_UPDATING
			elseif (IsRetail and TPerl_Party_CanAccess(Partyhealth) and TPerl_Party_CanAccess(Partyhealthmax)
					and (Partyhealth == 1) and (Partyhealthmax == 1)) then
				reason = TPERL_LOC_UPDATING
	...
	if (reason) then
		if (pconf.percent) then
			...
			self.statsFrame.healthBar.percent:SetText(reason)
				...
				-- Midnight/Retail: font and frame metrics can become secret values in tainted paths.
				-- Never compare/arithmetic on secret numbers. If we can't safely measure, fall back to the safer text placement.
				local tooWide
				if (IsRetail) then
					local ok, res = pcall(function()
						local wText = self.statsFrame.healthBar.percent:GetStringWidth()
						local wFrame = self.statsFrame:GetWidth()
						local wBar = self.statsFrame.healthBar:GetWidth()
						if (not TPerl_Party_CanAccess(wText) or not TPerl_Party_CanAccess(wFrame) or not TPerl_Party_CanAccess(wBar)) then
							return true
						end
						return wText > (wFrame - wBar - 8)
					end)
					tooWide = ok and res or true
				else
					tooWide = (self.statsFrame.healthBar.percent:GetStringWidth() > (self.statsFrame:GetWidth() - self.statsFrame.healthBar:GetWidth() - 8))
				end
...
local function TPerl_Party_UpdatePlayerFlags(self)
	local change
	if (TPerl_Party_SafeBool(UnitIsAFK(self.partyid)) and conf.showAFK) then
		if (not self.afk) then
			change = true
			self.afk = GetTime()
			self.dnd = nil
		end
	elseif (TPerl_Party_SafeBool(UnitIsDND(self.partyid))) then
		if (self.afk) then
			change = true
			self.afk = nil
		end
...
local function TPerl_Party_BuffPositions(self)
	-- NOTE: Party buff buttons can lose their anchors when layout/config refreshes
	-- (e.g. opening and closing the options window). If we only re-position when
	-- wrap is enabled, icons may end up with no points and appear to "disappear"
	-- until a full client restart recreates them. Always re-position here.
	if (self.conf) then
		if (pconf.debuffs.below) then
			TPerl_Unit_BuffPositions(self, self.buffFrame.buff, self.buffFrame.debuff, self.conf.buffs.size, self.conf.debuffs.size)
		else
			-- Debuffs handled separately by legacy code, so just do buffs here
...
local function TPerl_Party_Buff_UpdateAll(self)
	...
	if (self.conf) then
		if (not pconf.buffs.enable and not pconf.debuffs.enable) then
			...
			-- Some configs store disabled flags as 0 (e.g. after opening the options UI).
			-- In Lua, 0 is truthy, so normalize to booleans before passing into the buff updater.
			local castable = (pconf.buffs and (pconf.buffs.castable == 1 or pconf.buffs.castable == true)) or false
			local curable = (pconf.debuffs and (pconf.debuffs.curable == 1 or pconf.debuffs.curable == true)) or false
			TPerl_Unit_UpdateBuffs(self, nil, nil, castable, curable)
...
local function TPerl_Party_UpdateMana(self)
	...
	-- Refresh AFK flags if they changed (Retail safe).
	if (self.afk and not TPerl_Party_SafeBool(UnitIsAFK(partyid))) then
		TPerl_Party_UpdatePlayerFlags(self)
	end
	...
	-- Always try to paint the bar. In Midnight/Retail, UnitPower values may be "secret",
	-- but the StatusBar methods often still accept them.
	TPerl_Party_TrySetPowerBar(self.statsFrame.manaBar, unitPower, unitPowerMax)

	-- Midnight/Retail: UnitPower values can be returned as "secret" numbers.
	-- We still want to display values when possible, but must avoid direct comparisons/arithmetics
	-- on secret numbers outside protected calls.
	local powerPercent = 0
	local okMath = true
	local adjPowerMax = unitPowerMax

	local function calcPercent(up, um)
		-- Begin 4.3 division by 0 work around to ensure we don't divide if max is 0
		local pp
		if up > 0 and um == 0 then
			um = up
			pp = 1
		elseif up == 0 and um == 0 then
			pp = 0
		else
			pp = up / um
		end
	...
	if IsRetail then
		local ok, um, pp = pcall(calcPercent, unitPower, unitPowerMax)
		if ok then
			adjPowerMax = um
			powerPercent = pp
		else
		...
		adjPowerMax, powerPercent = calcPercent(unitPower, unitPowerMax)
	end

	-- Update percent/current display (Retail safe)
	if powerType >= 1 then
		local ok = true
		if IsRetail then
			ok = pcall(self.statsFrame.manaBar.percent.SetText, self.statsFrame.manaBar.percent, unitPower)
		else
		...
		elseif IsRetail then
			ok = pcall(self.statsFrame.manaBar.percent.SetFormattedText, self.statsFrame.manaBar.percent, percD, 100 * powerPercent)
		else
	...
	-- Respect the "Values" option for party frames.
	-- Try to format values even if the numbers are secret (inside pcall on Retail).
	if (pconf.values) then
		local ok = true
		if IsRetail then
			ok = pcall(self.statsFrame.manaBar.text.SetFormattedText, self.statsFrame.manaBar.text, "%d/%d", unitPower, adjPowerMax)
		else
			self.statsFrame.manaBar.text:SetFormattedText("%d/%d", unitPower, adjPowerMax)
		end
		if not ok then
	...
	-- Connection status (Retail safe): if the boolean is blocked/secret, assume connected.
	local connected = UnitIsConnected(partyid)
	local isConnected = true
	if (not IsRetail) then
		isConnected = (connected and true or false)
	elseif (TPerl_Party_CanAccess(connected)) then
		isConnected = (connected and true or false)
	end
...
local function TPerl_Party_TargetUpdateHealth(self)
	...
	-- Midnight (Retail): health values for party targets can be returned as secret values.
	-- Avoid compares/arithmetic on secret values; use UnitHealthPercent when accessible,
	-- otherwise drive the statusbar directly via pcall using raw hp/hpMax.
	local percent
	local canHp = TPerl_Party_CanAccess(hp)
	local canHpMax = TPerl_Party_CanAccess(hpMax)

	local function SafeSet(bar, minV, maxV, val)
		local ok1 = pcall(bar.SetMinMaxValues, bar, minV, maxV)
		if not ok1 then
			return false
		end
		local ok2 = pcall(bar.SetValue, bar, val)
		return ok2
	end

	if IsRetail and (not canHp or not canHpMax) then
		-- In Midnight/Retail, party target health can be hidden (secret). We can only show it when the value is accessible.
		-- First try UnitHealthPercent. If that is also secret, fall back to accessible absolute values (securecall),
		-- or, when the party target is our own target, use the "target" unit (usually accessible).
		-- Finally, keep last known accessible health instead of showing 0/dead.
		local function SafeUnitIsUnit(u1, u2)
			local ok, res = pcall(UnitIsUnit, u1, u2)
			if not ok then
				return false
			end
			return (TPerl_Party_CanAccess(res) and res) or false
		end

		local function GetAccessibleHealth(u)
			local h = UnitHealth(u)
			local hm = UnitHealthMax(u)
			if securecallfunction then
				local ok, v = pcall(securecallfunction, UnitHealth, u)
				if ok then h = v end
				ok, v = pcall(securecallfunction, UnitHealthMax, u)
				if ok then hm = v end
			end
			if TPerl_Party_CanAccess(h) and TPerl_Party_CanAccess(hm) and hm and hm > 0 then
				return h, hm
			end
		end

		local p = UnitHealthPercent and UnitHealthPercent(targetid, false) or nil
		if TPerl_Party_CanAccess(p) then
			percent = p
			tf.healthBar.text:SetFormattedText(percD, 100 * percent)
			SafeSet(tf.healthBar, 0, 1, percent)
		else
			local h2, hm2 = GetAccessibleHealth(targetid)
			if (not h2 or not hm2) and SafeUnitIsUnit(targetid, "target") then
				h2, hm2 = GetAccessibleHealth("target")
			end

			if h2 and hm2 then
				percent = h2 / hm2
				tf.lastHPGood, tf.lastHPMaxGood = h2, hm2
				tf.healthBar.text:SetFormattedText(percD, 100 * percent)
				SafeSet(tf.healthBar, 0, hm2, h2)
			elseif tf.lastHPGood and tf.lastHPMaxGood then
				tf.healthBar.text:SetText("")
				SafeSet(tf.healthBar, 0, tf.lastHPMaxGood, tf.lastHPGood)
			else
				tf.healthBar.text:SetText("")
				-- Unknown/secret: avoid looking dead. Show full bar with no numbers.
				if not SafeSet(tf.healthBar, 0, 1, 1) then
					SafeSet(tf.healthBar, 0, 1, 0)
				end
			end
		end
	else
		...
		elseif hpMax == 0 and hp and hp > 0 then -- We have current hp but max hp failed.
		...
		if (hpMax > 0 and percent) then
			...
			-- Clear percent text when target has no valid max health (e.g. no target) to avoid stale display.
			tf.healthBar.text:SetText("")
			tf.healthBar:SetMinMaxValues(0, 1)
			tf.healthBar:SetValue(0)
		end
	end
	...
	if (UnitIsDeadOrGhost(targetid)) then
		...
		if ((not IsRetail or canHpMax) and percent and hpMax > 0) then
...
local function TPerl_Party_UpdateTarget(self)
	if (pconf.target.enable) then
		...
		if (targetid and UnitIsConnected(partyid) and UnitExists(partyid) and UnitIsVisible(partyid)) then
			-- If the party member has no target, clear the display so we don't show stale data.
			local targetExists = UnitExists(targetid)
			if (not TPerl_Party_SafeBool(targetExists)) then
				self.targetFrame.text:SetText("")
				if self.targetFrame.healthBar and self.targetFrame.healthBar.text then
					self.targetFrame.healthBar.text:SetText("")
				end
				if self.targetFrame.healthBar then
					self.targetFrame.healthBar:SetMinMaxValues(0, 1)
					self.targetFrame.healthBar:SetValue(0)
				end
			...
			-- Midnight/Retail: UnitName() can return a "secret" string under taint.
			-- Never compare secret strings or use them in strict comparisons.
			if (targetname and TPerl_Party_CanAccess(targetname) and targetname ~= UNKNOWNOBJECT) then
				...
				-- Name missing/unknown/secret: clear the name to avoid showing stale targets.
				self.targetFrame.text:SetText("")
				TPerl_Party_TargetUpdateHealth(self)
				TPerl_Party_TargetRaidIcon(self)
			end
...
function TPerl_Party_OnUpdate(self, elapsed)
	...
	-- Aura refresh safety net (Midnight/Retail):
	-- Opening/closing options or UI refreshes can drop aura anchors or prevent UNIT_AURA from firing for these frames.
	-- Poll lightly to ensure party buffs/debuffs and debuff highlights stay in sync without requiring a full relog.
	if (IsRetail and self.conf and (pconf.buffs.enable or pconf.debuffs.enable or self._forceAuraRefresh)) then
		self._auraPoll = (self._auraPoll or 0) - elapsed
		if (self._auraPoll <= 0) then
			self._auraPoll = 0.75
			if (UnitExists(partyid)) then
				TPerl_Party_Buff_UpdateAll(self)
			end
			self._forceAuraRefresh = nil
		end
	end
	...
	--if (self.time >= 0.2) then
		...
		if (pconf.target.large and self.targetFrame:IsShown()) then
			...
			if (GetTime() > self.targetFrame.lastUpdate + 5000) then
				needsUpdate = true
			elseif IsRetail and (
				not TPerl_Party_CanAccess(hp) or not TPerl_Party_CanAccess(hpMax) or
				not TPerl_Party_CanAccess(self.targetFrame.lastHP) or not TPerl_Party_CanAccess(self.targetFrame.lastHPMax) or
				not TPerl_Party_CanAccess(heal) or not TPerl_Party_CanAccess(self.targetFrame.lastHeal) or
				not TPerl_Party_CanAccess(absorb) or not TPerl_Party_CanAccess(self.targetFrame.lastAbsorb)
			) then
				-- Can't safely compare (secret values), so refresh occasionally.
				needsUpdate = (GetTime() > self.targetFrame.lastUpdate + 1)
			else
				needsUpdate = (hp ~= self.targetFrame.lastHP or hpMax ~= self.targetFrame.lastHPMax or heal ~= self.targetFrame.lastHeal or absorb ~= self.targetFrame.lastAbsorb)
			end
			if (needsUpdate) then
				TPerl_Party_TargetUpdateHealth(self)
			end
