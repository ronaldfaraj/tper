...
CreateFrame("Frame", "TPerl_Voice", BackdropTemplateMixin and "BackdropTemplate")
local voice = TPerl_Voice
...
function voice:ClearCache()
	self.cachedUnits = setmetatable({}, {
		__index = function(self, unit)
			for frame in pairs(TPerl_Voice.frames) do
...
function voice:Create(frame)
	...
	TPerl_SetChildMembers(frame.voiceButton)
	self:Position(frame)
end
...
function voice:UpdateVoice(frame, onoff)
	...
	if (onoff) then
		...
		TPerl_FrameFlash(frame.voiceButton.Flash)
	else
		if frame.voiceButton then
			frame.voiceButton.On:Hide()
			TPerl_FrameFlashStop(frame.voiceButton.Flash)
		end
	end
