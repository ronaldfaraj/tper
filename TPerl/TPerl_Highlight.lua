...
local pomSpells = TPERL_HIGHLIGHT_SPELLS.pomSpells
local shieldSpells = TPERL_HIGHLIGHT_SPELLS.shieldSpells
...
local xpHigh = CreateFrame("Frame", "TPerl_Highlight", nil, BackdropTemplateMixin and "BackdropTemplate")
xpHigh.list = {}
xpHigh.callbacks = {}
...
function xpHigh:TooltipInfo(guid)
	...
	if (effects) then
		local str = ""
		local prefix = TPERL_LOC_STATUSTIP
		for k,v in pairs(effects) do
			local desc = TPERL_LOC_STATUSTIPLIST[k]
			local c = colours[k]
			if (desc and c) then
...
function xpHigh:SetHighlight(frame, guid)
	if (not frame.highlight) then
		TPerl_Notice("No .highlight region for "..frame:GetName()..", unit: "..guid)
		return
	end
		...
		if (r) then
			...
			for k, v in pairs(r) do
				...
				elseif (k == "HOTSPARKS" and conf.highlight.extraSparkles) then
					...
					if (not r1 or t1 == "TARGET") then
						if frame == TPerl_Player or frame == TPerl_Player_Pet or frame == TPerl_Target or frame == TPerl_TargetTarget or frame == TPerl_TargetTargetTarget or frame == TPerl_Focus or frame == TPerl_FocusTarget or frame == TPerl_partypet1 or frame == TPerl_partypet2 or frame == TPerl_partypet3 or frame == TPerl_partypet4 or frame == TPerl_partypet5 or strmatch(frame:GetName(), "TPerl_Raid_GrpPetsUnitButton") then
						else
							t1 = k
			...
			if (r1) then
				...
				if (self.flashers[guid]) then
					TPerl_FrameFlash(frame.highlight.tex)
				else
					TPerl_FrameFlashStop(frame.highlight.tex)
				end
...
function xpHigh:CreateHotCount(frame)
	...
	if (sf) then
		if (strfind(frame:GetName(), "TPerl_Raid_Grp")) then
			f:SetTextHeight(15)
		...
		if (strfind(frame:GetName(), "TPerl_Raid_GrpPets")) then
			f:SetTextHeight(15)
...
function xpHigh:CreateMendingIcon(frame)
	...
	if (p) then
		p = p.healthBar or p
		if (strfind(p:GetName(), "^TPerl_Raid")) then
			sizeMod = 0
			yOffset = -2
		else
			sizeMod = 10
...
function xpHigh:TriggerMendingAnimation(sourceGUID, targetGUID)
	...
	if (IsInRaid() and TPerl_Raid_GetUnitFrameByUnit) then
		sourceFrame, targetFrame = TPerl_Raid_GetUnitFrameByGUID(sourceGUID), TPerl_Raid_GetUnitFrameByGUID(targetGUID)
		if (not sourceFrame and TPerl_Raid_Pet_GetUnitFrameByGUID) then
			sourceFrame = TPerl_Raid_Pet_GetUnitFrameByGUID(sourceGUID)
		end
		if (not targetFrame and TPerl_Raid_Pet_GetUnitFrameByGUID) then
			targetFrame = TPerl_Raid_Pet_GetUnitFrameByGUID(targetGUID)
		end
	elseif (GetNumSubgroupMembers() > 0 and TPerl_Party_GetUnitFrameByUnit) then
		sourceFrame = sourceGUID == UnitGUID("player") and TPerl_Player or TPerl_Party_GetUnitFrameByGUID(sourceGUID)
		targetFrame = targetGUID == UnitGUID("player") and TPerl_Player or TPerl_Party_GetUnitFrameByGUID(targetGUID)
		if (not sourceFrame and TPerl_Party_Pet_GetUnitFrameByGUID) then
			sourceFrame = TPerl_Party_Pet_GetUnitFrameByGUID(sourceGUID)
		end
		if (not targetFrame and TPerl_Party_Pet_GetUnitFrameByGUID) then
			targetFrame = TPerl_Party_Pet_GetUnitFrameByGUID(targetGUID)
		end
	end
...
function xpHigh:SparkleAreaAddSpark(area, bias)
	...
	if (not spark) then
		-- Create new spark
		spark = CreateFrame("Frame", nil, TPerl_Highlight, BackdropTemplateMixin and "BackdropTemplate")
...
-- TPerl_RegisterFrameHighlighter
...
function xpHigh:FindMyPomPom()
	for unit, unitName, unitClass, group in TPerl_NextMember do
...
function xpHigh:RefreshAllAuras()
	self.expectingPOM = nil
	for unitid in TPerl_NextMember do
