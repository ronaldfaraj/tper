...
TPerlRaidMonConfig = {}
local config = TPerlRaidMonConfig
...
TPERL_RAIDMON_UNIT_WIDTH_MIN = 50
TPERL_RAIDMON_UNIT_WIDTH_MAX = 150
TPERL_RAIDMON_UNIT_HEIGHT_MIN = 9
TPERL_RAIDMON_UNIT_HEIGHT_MAX = 20
TPERL_RAIDMON_TARGET_WIDTH_MIN = 50
TPERL_RAIDMON_TARGET_WIDTH_MAX = 150
...
-- TPerl_RaidMonitor_OnLoad
function TPerl_RaidMonitor_OnLoad(self)
	cast = self
	TPerl_RaidMonitor_Init(self)

	--self:RegisterEvent("VARIABLES_LOADED")
	self:RegisterEvent("PLAYER_ENTERING_WORLD")
	self:RegisterEvent("GROUP_ROSTER_UPDATE")
	self:SetScript("OnEvent", TPerl_RaidMonitor_OnEvent)
	self:SetScript("OnUpdate", self.OnUpdate)

	if IsRetail then
		TPerl_RaidMonitor_Frame_TitleBarCloseButton:SetScale(0.66)
		TPerl_RaidMonitor_Frame_TitleBarCloseButton:SetPoint("TOPRIGHT", 2, 2)
		TPerl_RaidMonitor_Frame_TitleBarPin:SetPoint("RIGHT", TPerl_RaidMonitor_Frame_TitleBarCloseButton, "LEFT", 0, 0)
		TPerl_RaidMonitor_Frame_TitleBarTotals:SetPoint("RIGHT", TPerl_RaidMonitor_Frame_TitleBarPin, "LEFT", 0, 0)
	end

	if (TPerl_RegisterPerlFrames) then
		TPerl_RegisterPerlFrames(TPerl_RaidMonitor_Frame)
	end

	if (TPerl_SavePosition) then
		TPerl_SavePosition(TPerl_RaidMonitor_Anchor, true)
	end

	TPerl_RegisterScalableFrame(self, TPerl_RaidMonitor_Anchor)

	TPerl_RaidMonitor_OnLoad = nil
end
...
local function UpdateTarget(self)
	...
	if (id) then
		...
		elseif (not UnitIsVisible(subid)) then
			reason = TPERL_LOC_OOR
		else
			name = UnitName(id)
			if (not name) then
				reason = TPERL_NO_TARGET
			end
		end
...
function TPerl_CastMon_ChildUnits(self)
	...
	TPerl_RegisterClickCastFrame(self.target)
	...
	if (class == "DRUID") then
		...
		self:SetAttribute("spell2", TPERL_MONITOR_INNERVATE)
	elseif (class == "SHAMAN") then
		self:SetAttribute("type2", "spell")
		self:SetAttribute("spell2", TPERL_MONITOR_MANATIDE)
	end
...
-- cast:OnEvent
function TPerl_RaidMonitor_OnEvent(self, event, ...)
	local e = self[event]
	if (e) then
		e(self, ...)
	end
...
-- TPerl_Monitor_MakeAction
function TPerl_Monitor_MakeAction(self, str, type)
	if (str == "target") then
		return TPERL_MONITOR_CLICKTARGET
	elseif (str == "spell") then
		...
		if (spell) then
			return string.format(TPERL_MONITOR_CLICKCAST, spell)
		end
	end
end

-- TPerl_RaidMonitor_Init
function TPerl_RaidMonitor_Init(self)
	self.updateTimer = 0
	...
	function cast:PLAYER_ENTERING_WORLD()
		...
		self.area:SetAttribute("template", "TPerl_RaidMonitor_UnitTemplate")
	...
	function cast:SetUnitSizes()
		if (InCombatLockdown()) then
			TPerl_OutOfCombatQueue[cast.SetUnitSizes] = self
			return
		end
	...
	function cast:EnableDisable()
		...
		local CastbarEventHandler = function(event, ...)
			return TPerl_RaidMonitor_OnEvent(self, event, ...)
		end
	...
	function cast:SetFrameSizes()
		if (InCombatLockdown()) then
			TPerl_OutOfCombatQueue[cast.SetFrameSizes] = self
			return
		end
		...
		TPerl_RegisterScalableFrame(self, TPerl_RaidMonitor_Anchor, nil, nil, config.GrowUp)
		...
		if (config.GrowUp) then
			TPerl_SwitchAnchor(TPerl_RaidMonitor_Anchor, "BOTTOMLEFT")
			...
			TPerl_SwitchAnchor(TPerl_RaidMonitor_Anchor, "TOPLEFT")
