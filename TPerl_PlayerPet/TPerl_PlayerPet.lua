...
local TPerl_Player_Pet_Events = {}
local conf, pconf
TPerl_RequestConfig(function(new)
	conf = new
	pconf = new.pet
	if (TPerl_Player_Pet) then
		TPerl_Player_Pet.conf = pconf
	end
end, "$Revision:  $")
...
local TPerl_Player_Pet_HighlightCallback

-- TPerl_Player_Pet_OnLoad
function TPerl_Player_Pet_OnLoad(self)
	TPerl_SetChildMembers(self)
	self.partyid = "pet"

	TPerl_BlizzFrameDisable(PetFrame)
	...
	self.buffSetup = {
		...
		debuffAnchor1 = function(self, b)
			local buff1 = TPerl_GetBuffButton(self, 1, 0, true)
	...
	--TPerl_SecureUnitButton_OnLoad(self, "pet", nil, PetFrameDropDown, TPerl_ShowGenericMenu)			--PetFrame.menu)
	--TPerl_SecureUnitButton_OnLoad(self.nameFrame, "pet", nil, PetFrameDropDown, TPerl_ShowGenericMenu)	--PetFrame.menu)
	...
	self.state:SetFrameRef("TPerlPlayerPet", self)

	self.state:Execute([[
		frame = self:GetFrameRef("TPerlPlayerPet")
	]])
	...
	self:SetScript("OnEvent", TPerl_Player_Pet_OnEvent)
	self:SetScript("OnUpdate", TPerl_Player_Pet_OnUpdate)
	--[[if (FOM_FeedButton) then
		self:SetScript("OnShow", function(self)
			TPerl_Unit_UpdatePortrait(self)
			TPerl_ProtectedCall(Set_FOM_FeedButton)
		end)
	else
	...
	if (TPerl_ArcaneBar_RegisterFrame) then
		TPerl_ArcaneBar_RegisterFrame(self.nameFrame, (not IsVanillaClassic and UnitHasVehicleUI("player")) and "player" or "pet")
	end
	...
	TPerl_Player_Pet_OnLoad = nil
end

-- TPerl_Player_Pet_HighlightCallback
function TPerl_Player_Pet_HighlightCallback(self, updateGUID)
	if (updateGUID == UnitGUID("pet")) then
		TPerl_Highlight:SetHighlight(self, updateGUID)
	end
end

-- TPerl_Player_Pet_UpdateName
local function TPerl_Player_Pet_UpdateName(self)
...
-- TPerl_Player_Pet_UpdateLevel
local function TPerl_Player_Pet_UpdateLevel(self)
	TPerl_Unit_UpdateLevel(self)
end

-- TPerl_Player_Pet_UpdateAbsorbPrediction
local function TPerl_Player_Pet_UpdateAbsorbPrediction(self)
...
-- TPerl_Player_Pet_UpdateHotsPrediction
local function TPerl_Player_Pet_UpdateHotsPrediction(self)
	if pconf.absorbs then
...
-- TPerl_Player_Pet_UpdateHealPrediction
local function TPerl_Player_Pet_UpdateHealPrediction(self)
...
local function TPerl_Player_Pet_UpdateResurrectionStatus(self)
...
-- TPerl_Player_Pet_UpdateHealth
local function TPerl_Player_Pet_UpdateHealth(self)
	local partyid = self.partyid
	...
	TPerl_SetHealthBar(self, pethealth, pethealthmax)

	TPerl_Player_Pet_UpdateAbsorbPrediction(self)
	TPerl_Player_Pet_UpdateHotsPrediction(self)
	TPerl_Player_Pet_UpdateHealPrediction(self)
	TPerl_Player_Pet_UpdateResurrectionStatus(self)
...
-- TPerl_Player_Pet_UpdateMana()
local function TPerl_Player_Pet_UpdateMana(self)
	local partyid = self.partyid
...
-- TPerl_Player_Pet_CombatFlash
local function TPerl_Player_Pet_CombatFlash(self, elapsed, argNew, argGreen)
	if (TPerl_CombatFlashSet(self, elapsed, argNew, argGreen)) then
		TPerl_CombatFlashSetFrames(self)
	end
end

-- TPerl_Player_Pet_OnUpdate
function TPerl_Player_Pet_OnUpdate(self, elapsed)
	if pconf.hitIndicator and pconf.portrait then
		CombatFeedback_OnUpdate(self, elapsed)
	end
	if (self.PlayerFlash) then
		TPerl_Player_Pet_CombatFlash(self, elapsed, false)
	end
end
...
-- Buff Functions --
--------------------
local function TPerl_Player_Pet_Buff_UpdateAll(self)
	local partyid = self.partyid
...
-- Happiness --
---------------
local function TPerl_Player_Pet_SetHappiness(self)
 ...
 local _, classFileName = UnitClass("player")
	...
	if (pconf.happiness.enable and (not pconf.happiness.onlyWhenSad or happiness < 3)) then
		...
		if (pconf.happiness.flashWhenSad and happiness < 3) then
			TPerl_FrameFlash(self.happyFrame)
		else
			TPerl_FrameFlashStop(self.happyFrame)
		end
	else
		TPerl_FrameFlashStop(self.happyFrame)
...
-- TPerl_Player_Pet_Update_Control
local function TPerl_Player_Pet_Update_Control(self)
...
-- TPerl_Player_Pet_UpdateCombat
local function TPerl_Player_Pet_UpdateCombat(self)
	local partyid = self.partyid
	if (UnitExists(partyid)) then
		...
		TPerl_Player_Pet_Update_Control(self)
	end
end

-- TPerl_Player_Pet_UpdateDisplay
function TPerl_Player_Pet_UpdateDisplay(self)
	...
	TPerl_Unit_UpdatePortrait(self)
	TPerl_Player_Pet_UpdateName(self)
	TPerl_Player_Pet_UpdateHealth(self)
	TPerl_SetManaBarType(self)
	if (TPerl_Player_Pet_SetHappiness) then
		TPerl_Player_Pet_SetHappiness(self)
	end
	TPerl_Player_Pet_UpdateMana(self)
	TPerl_Player_Pet_UpdateLevel(self)
	TPerl_Player_Pet_Buff_UpdateAll(self)
	TPerl_Player_Pet_UpdateCombat(self)
end

-------------------
-- Event Handler --
-------------------
function TPerl_Player_Pet_OnEvent(self, event, unit, ...)
	if string.find(event, "^UNIT_") then
		if (unit == "pet" or unit == "player") then
			if event == "UNIT_HEAL_PREDICTION" or event == "UNIT_ABSORB_AMOUNT_CHANGED" or event == "UNIT_COMBAT"  then
				TPerl_Player_Pet_Events[event](self, unit, ...)
			else
				TPerl_Player_Pet_Events[event](self, ...)
			end
		end
	else
		TPerl_Player_Pet_Events[event](self, event, unit, ...)
	end
end

-- VARIABLES_LOADED
function TPerl_Player_Pet_Events:VARIABLES_LOADED()
	local _, classFileName = UnitClass("player")
	if not classFileName == "HUNTER" then
		TPerl_Player_Pet_Events.UNIT_HAPPINESS = nil
		TPerl_Player_Pet_SetHappiness = nil
	end

	TPerl_Player_Pet_Events.VARIABLES_LOADED = nil
end

-- UNIT_AURA
function TPerl_Player_Pet_Events:UNIT_AURA()
	TPerl_Player_Pet_Buff_UpdateAll(self)
end

-- UNIT_PET
function TPerl_Player_Pet_Events:UNIT_PET()
	if (conf) then		-- DK can issue very early UNIT_PET, long before PEW. We refresh on entering world regardless
		...
		TPerl_Player_Pet_UpdateDisplay(self)
	end
end

--[[function TPerl_Player_Pet_Events:PET_BATTLE_OPENING_START()
...
function TPerl_Player_Pet_Events:PET_BATTLE_CLOSE()
	if (UnitExists("pet")) then
		if not InCombatLockdown() then
			RegisterUnitWatch(self)
		end
		TPerl_ProtectedCall(Show, self)
	end
end]]

TPerl_Player_Pet_Events.PET_STABLE_SHOW = TPerl_Player_Pet_Events.UNIT_PET

-- UNIT_NAME_UPDATE
function TPerl_Player_Pet_Events:UNIT_NAME_UPDATE()
	TPerl_Player_Pet_UpdateName(self)
end

-- UNIT_PORTRAIT_UPDATE
function TPerl_Player_Pet_Events:UNIT_PORTRAIT_UPDATE()
	TPerl_Unit_UpdatePortrait(self, true)
end

-- UNIT_HEALTH_FREQUENT
function TPerl_Player_Pet_Events:UNIT_HEALTH_FREQUENT()
	TPerl_Player_Pet_UpdateHealth(self)
end

-- UNIT_HEALTH
function TPerl_Player_Pet_Events:UNIT_HEALTH()
	TPerl_Player_Pet_UpdateHealth(self)
end

-- UNIT_MAXHEALTH
function TPerl_Player_Pet_Events:UNIT_MAXHEALTH()
	TPerl_Player_Pet_UpdateHealth(self)
end

function TPerl_Player_Pet_Events:UNIT_POWER_FREQUENT()
	TPerl_Player_Pet_UpdateMana(self)
	TPerl_Player_Pet_UpdateCombat(self)
end

function TPerl_Player_Pet_Events:UNIT_MAXPOWER()
	TPerl_Player_Pet_UpdateMana(self)
	TPerl_Player_Pet_UpdateCombat(self)
end

-- UNIT_LEVEL
function TPerl_Player_Pet_Events:UNIT_LEVEL()
	TPerl_Player_Pet_UpdateLevel(self)
end

-- UNIT_DISPLAYPOWER
function TPerl_Player_Pet_Events:UNIT_DISPLAYPOWER()
	TPerl_SetManaBarType(self)
end

-- UNIT_HAPPINESS
function TPerl_Player_Pet_Events:UNIT_HAPPINESS()
	TPerl_Player_Pet_SetHappiness(self)
	TPerl_Player_Pet_UpdateCombat(self)
end

-- PET_ATTACK_START
function TPerl_Player_Pet_Events:PET_ATTACK_START()
	TPerl_Player_Pet_UpdateCombat(self)
end

-- UNIT_COMBAT
function TPerl_Player_Pet_Events:UNIT_COMBAT(unit, action, descriptor, damage, damageType)
	if unit ~= self.partyid then
		return
	end
	...
	if (action == "HEAL") then
		TPerl_Player_Pet_CombatFlash(TPerl_Player_Pet, 0, true, true)
	elseif (damage and damage > 0) then
		TPerl_Player_Pet_CombatFlash(TPerl_Player_Pet, 0, true)
	end
end

-- UNIT_FACTION
function TPerl_Player_Pet_Events:UNIT_FACTION()
	TPerl_Player_Pet_UpdateName(self)
	TPerl_Player_Pet_UpdateCombat(self)
end

TPerl_Player_Pet_Events.UNIT_FLAGS = TPerl_Player_Pet_Events.UNIT_FACTION

-- PLAYER_REGEN_ENABLED
function TPerl_Player_Pet_Events:PLAYER_REGEN_ENABLED()
...
-- PLAYER_ENTERING_WORLD
function TPerl_Player_Pet_Events:PLAYER_ENTERING_WORLD()
...
-- UNIT_ENTERED_VEHICLE
function TPerl_Player_Pet_Events:UNIT_ENTERED_VEHICLE(showVehicle)
	if showVehicle then
		self.partyid = "player"
		self.unit = self.partyid
...
-- UNIT_EXITING_VEHICLE
function TPerl_Player_Pet_Events:UNIT_EXITING_VEHICLE()
	if (self.partyid ~= "pet") then
		self.partyid = "pet"
		self.unit = self.partyid
		if (TPerl_ArcaneBar_SetUnit) then
			TPerl_ArcaneBar_SetUnit(self.nameFrame, "pet")
		end
		--[[if (not InCombatLockdown()) then
			self:SetAttribute("unit", "pet")
		end]]
...
function TPerl_Player_Pet_Events:UNIT_THREAT_LIST_UPDATE()
	TPerl_Unit_ThreatStatus(self)
end

function TPerl_Player_Pet_Events:PLAYER_TARGET_CHANGED()
	TPerl_Unit_ThreatStatus(self, nil, true)
end
TPerl_Player_Pet_Events.UNIT_TARGET = TPerl_Player_Pet_Events.PLAYER_TARGET_CHANGED

-- PLAYER_REGEN_DISABLED
local virtual
function TPerl_Player_Pet_Events:PLAYER_REGEN_DISABLED()
	if virtual then
		virtual = nil
		RegisterUnitWatch(TPerl_Player_Pet)
		if (UnitExists("pet")) then
			TPerl_Player_Pet:Show()
			TPerl_Player_Pet_UpdateDisplay(TPerl_Player_Pet)
		else
			TPerl_Player_Pet:Hide()
		end

		if (TPerl_PetTarget) then
			RegisterUnitWatch(TPerl_PetTarget)
...
function TPerl_Player_Pet_Events:UNIT_HEAL_PREDICTION(unit)
...
function TPerl_Player_Pet_Events:UNIT_ABSORB_AMOUNT_CHANGED(unit)
	if pconf.absorbs and unit == self.partyid then
		TPerl_SetExpectedAbsorbs(self)
	end
end

function TPerl_Player_Pet_Events:INCOMING_RESURRECT_CHANGED(unit)
	if unit == self.partyid then
		TPerl_Player_Pet_UpdateResurrectionStatus(self)
	end
end

-- TPerl_Player_Pet_SetWidth
function TPerl_Player_Pet_SetWidth(self)
	...
	TPerl_StatsFrameSetup(self, nil, 2)
	TPerl_SavePosition(self, true)
end

-- TPerl_Player_Pet_Virtual
function TPerl_Player_Pet_Virtual(show)
	if (not InCombatLockdown()) then
		if (show) then
			if (not virtual) then
				virtual = true
				if not UnitExists("pet") then
					TPerl_Player_Pet:RegisterEvent("PLAYER_REGEN_DISABLED")
					UnregisterUnitWatch(TPerl_Player_Pet)
					TPerl_Player_Pet:Show()
					TPerl_Player_Pet.nameFrame.text:SetText(PET)
					TPerl_Player_Pet.statsFrame.healthBar.text:SetText("")
					TPerl_Player_Pet.statsFrame.manaBar.text:SetText("")
					TPerl_Player_Pet:SetAlpha(0.5)
				end

				if TPerl_PetTarget and not UnitExists("pettarget") then
					TPerl_PetTarget.virtual = true
					UnregisterUnitWatch(TPerl_PetTarget)
					TPerl_PetTarget:Show()
					TPerl_PetTarget.nameFrame.text:SetText(PET.." "..TARGET)
					TPerl_PetTarget.statsFrame.healthBar.text:SetText("")
					TPerl_PetTarget.statsFrame.manaBar.text:SetText("")
					TPerl_PetTarget.statsFrame.healthBar.percent:SetText("")
					TPerl_PetTarget.statsFrame.manaBar.percent:SetText("")
					TPerl_PetTarget:SetAlpha(0.5)
				end
			end
		else
			if (virtual) then
				...
				RegisterUnitWatch(TPerl_Player_Pet)
				if (TPerl_PetTarget) then
					TPerl_PetTarget.virtual = nil
					if (TPerl_PetTarget.conf.enable) then
						RegisterUnitWatch(TPerl_PetTarget)
						TPerl_PetTarget:Hide()
					end
				end
				TPerl_Player_Pet:UnregisterEvent("PLAYER_REGEN_DISABLED")
			end
		end
	end
end
...
function TPerl_Player_Pet_Set_Bits(self)
	...
	TPerl_Player_Pet_SetWidth(self)
