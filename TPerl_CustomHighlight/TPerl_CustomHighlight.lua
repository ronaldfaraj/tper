...
if (not TPerl_RequestConfig) then
	return
end
...
local CustomHighlight = CreateFrame("Frame", "TPerl_Custom")
...
CustomHighlight.RaidFrameArray = TPerl_Raid_GetFrameArray()

-- Pull zone names from encounter journal so they don't need to be localized.
-- Cataclysm Raids
local TPERL_LOC_ZONE_ANTORUS = not IsClassic and EJ_GetInstanceInfo(946) or TPERL_LOC_ZONE_ANTORUS
local TPERL_LOC_ZONE_BARADIN_HOLD = not IsClassic and EJ_GetInstanceInfo(75) or TPERL_LOC_ZONE_BARADIN_HOLD
local TPERL_LOC_ZONE_BASTION_OF_TWILIGHT = not IsClassic and EJ_GetInstanceInfo(72) or TPERL_LOC_ZONE_BASTION_OF_TWILIGHT
local TPERL_LOC_ZONE_BLACK_TEMPLE = not IsClassic and EJ_GetInstanceInfo(751) or TPERL_LOC_ZONE_BLACK_TEMPLE
local TPERL_LOC_ZONE_BLACKWING_DECENT = not IsClassic and EJ_GetInstanceInfo(73) or TPERL_LOC_ZONE_BLACKWING_DECENT
local TPERL_LOC_ZONE_DRAGONSOUL = not IsClassic and EJ_GetInstanceInfo(187) or TPERL_LOC_ZONE_DRAGONSOUL
local TPERL_LOC_ZONE_EMERALD_NIGHTMARE = not IsClassic and EJ_GetInstanceInfo(768) or TPERL_LOC_ZONE_EMERALD_NIGHTMARE
local TPERL_LOC_ZONE_EYE_OF_ETERNITY = not IsClassic and EJ_GetInstanceInfo(946) or TPERL_LOC_ZONE_EYE_OF_ETERNITY
local TPERL_LOC_ZONE_FIRELANDS = not IsClassic and EJ_GetInstanceInfo(78) or TPERL_LOC_ZONE_FIRELANDS
local TPERL_LOC_ZONE_HYJAL_SUMMIT = not IsClassic and EJ_GetInstanceInfo(750) or TPERL_LOC_ZONE_HYJAL_SUMMIT
local TPERL_LOC_ZONE_ICECROWN_CITADEL = not IsClassic and EJ_GetInstanceInfo(758) or TPERL_LOC_ZONE_ICECROWN_CITADEL
local TPERL_LOC_ZONE_KARAZHAN = not IsClassic and EJ_GetInstanceInfo(745) or TPERL_LOC_ZONE_KARAZHAN
local TPERL_LOC_ZONE_NAXXRAMAS = not IsClassic and EJ_GetInstanceInfo(754) or TPERL_LOC_ZONE_NAXXRAMAS
local TPERL_LOC_ZONE_NIGHTHOLD = not IsClassic and EJ_GetInstanceInfo(786) or TPERL_LOC_ZONE_NIGHTHOLD
local TPERL_LOC_ZONE_OBSIDIAN_SANCTUM = not IsClassic and EJ_GetInstanceInfo(755) or TPERL_LOC_ZONE_OBSIDIAN_SANCTUM
local TPERL_LOC_ZONE_RUBY_SANCTUM = not IsClassic and EJ_GetInstanceInfo(761) or TPERL_LOC_ZONE_RUBY_SANCTUM
local TPERL_LOC_ZONE_SERPENTSHRINE_CAVERN = not IsClassic and EJ_GetInstanceInfo(748) or TPERL_LOC_ZONE_SERPENTSHRINE_CAVERN
local TPERL_LOC_ZONE_SUNWELL_PLATEAU = not IsClassic and EJ_GetInstanceInfo(752) or TPERL_LOC_ZONE_SUNWELL_PLATEAU
local TPERL_LOC_ZONE_THRONE_OF_FOUR_WINDS = not IsClassic and EJ_GetInstanceInfo(74) or TPERL_LOC_ZONE_THRONE_OF_FOUR_WINDS
local TPERL_LOC_ZONE_TOMB_OF_SARGERAS = not IsClassic and EJ_GetInstanceInfo(875) or TPERL_LOC_ZONE_TOMB_OF_SARGERAS
local TPERL_LOC_ZONE_TRIAL_OF_THE_CRUSADER = not IsClassic and EJ_GetInstanceInfo(757) or TPERL_LOC_ZONE_TRIAL_OF_THE_CRUSADER
local TPERL_LOC_ZONE_TRIAL_OF_VALOR = not IsClassic and EJ_GetInstanceInfo(861) or TPERL_LOC_ZONE_TRIAL_OF_VALOR
local TPERL_LOC_ZONE_ULDUAR = not IsClassic and EJ_GetInstanceInfo(759) or TPERL_LOC_ZONE_ULDUAR
--[[
-- Mists of Pandaria Raids
local TPERL_LOC_ZONE_MOGUSHAN_VAULTS = EJ_GetInstanceInfo(317)
local TPERL_LOC_ZONE_TERRACE_OF_ENDLESS_SPRING = EJ_GetInstanceInfo(320)
local TPERL_LOC_ZONE_HEART_OF_FEAR = EJ_GetInstanceInfo(1009)
--]]
-- DefaultZoneData
function CustomHighlight:DefaultZoneData()
	return {
		[TPERL_LOC_ZONE_ANTORUS] =
			{
		...
		[TPERL_LOC_ZONE_TOMB_OF_SARGERAS] =
			{
		...
		[TPERL_LOC_ZONE_SERPENTSHRINE_CAVERN] =
			{
				[38132] = true,								-- Paralyze - Picked up Tainted Core
			},
		[TPERL_LOC_ZONE_BLACK_TEMPLE] =
			{
		...
		[TPERL_LOC_ZONE_HYJAL_SUMMIT] =
			{
				[39941] = true,								-- Inferno
				[31347] = true,								-- Doom
			},
		[TPERL_LOC_ZONE_KARAZHAN] =
			{
				[34661] = true,								-- Sacrifice (Illhoof)
				[30753] = true,								-- Red Riding Hood (Big Bad Wolf)
			},
		[TPERL_LOC_ZONE_SUNWELL_PLATEAU] =
			{
		...
		[TPERL_LOC_ZONE_OBSIDIAN_SANCTUM] =
			{
		...
		[TPERL_LOC_ZONE_TRIAL_OF_THE_CRUSADER] =
			{
		...
		[TPERL_LOC_ZONE_ICECROWN_CITADEL] =
			{
...
--				[71154] = true,								-- Rend Flesh (Trash)
		...
		[TPERL_LOC_ZONE_BARADIN_HOLD] =
			{
				[88954] = true,								-- Consuming Darkness (Argaloth)
			},
		[TPERL_LOC_ZONE_BLACKWING_DECENT] =
			{
		...
		[TPERL_LOC_ZONE_BASTION_OF_TWILIGHT] =
			{
		...
		[TPERL_LOC_ZONE_THRONE_OF_FOUR_WINDS] =
			{
...
function CustomHighlight:SetDefaultZoneData()
	if conf then
		...
		TPerlDB.custom.zones = conf.zones
...
function CustomHighlight:UNIT_AURA(unit)
	...
	local frame = TPerl_Raid_GetUnitFrameByUnit(unit)
	if frame then
		self:Check(frame, unit)
	end
end
